...
class TemplateCreator(ABC):
    def __init__(self, lib, scheme, n2, n, rm_decoder, all_one_blocks, m_len, config,eindex):
        self.lib = lib
        self.scheme = scheme
        self.n2 = n2
        self.n = n
...
#        print(selected_error_patterns.shape,np.array(selected_error_patterns[0]))
    ...
    def decode_result(self, vector_sum):
        """Abstract method to decode the result based on the mode (MV or FD)."""
        pass
    ...
    def create_template(self, error_patterns):
        ...
        error_pattern = np.array(selected_patterns[int(self.eindex)])
        ...
        for decoded_result in list(u_aggregate.keys()):
            decoder_result_percentage = (results_distribution[decoded_result] / total_samples) * 100
            print(f'Error pattern {self.eindex}: decoder result {decoded_result}, percentage {decoder_result_percentage:.2f}%.')
            probability_template[decoded_result] = u_aggregate[decoded_result] / results_distribution[decoded_result]            
            # Filter out results not happening once out of 2000
            if decoder_result_percentage<1/2000*100:
                del u_aggregate[decoded_result]
                del results_distribution[decoded_result]
                del probability_template[decoded_result]

        # Save template
        self.save_probability_template_to_pickle(probability_template, self.eindex)
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python script.py <scheme><mvORfd><eindex>")
        sys.exit(1)
    
    main(scheme=sys.argv[1], mvORfd=sys.argv[2], eindex=sys.argv[3])
